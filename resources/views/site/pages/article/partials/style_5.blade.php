    ...
    <div class="entry-thumbnail" height="100%">
        ...
        @if($post->post_type=='video')
            @if($post->video_id != null)
                ...
                @if(isFileExist(@$post->videoThumbnail, @$post->videoThumbnail->original_image))

                    <video id='player_in' controls
                           poster="{{basePath($post->videoThumbnail)}}/{{ $post->videoThumbnail->original_image }} "
                           height="100%">
                @else
                    <video id='player_in' controls
                ...
                @else
                    ...
                    @elseif($post->video_url_type == "mp4_url")
                        <video id="player_in" playsinline controls
                               data-poster="{{ basePath(@$post->image)}}/{{  @$post->image->original_image}}"
                               height="100%">
